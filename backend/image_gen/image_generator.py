...
DEFAULT_API_KEY = "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"
...
⚠️ CRITICAL: If ANY of these checks fail, the image must be regenerated."""
        ...
        self.full_body_transform_prompt = """Create a professional fashion photo by transforming this base body image to wear the exact clothing and accessories from the reference fashion image.

COPY THE EXACT CLOTHING FROM THE REFERENCE IMAGE - DO NOT ADD OR CHANGE ANYTHING.

- ⚠️ CAREFULLY EXAMINE the reference image to see what clothing the model is actually wearing
- PRECISELY replicate all clothing items, textures, patterns, colors, and styling shown in the reference image
- MATCH the exact appearance including fabric textures, colors, fit, and styling details
- ENSURE all clothing details like buttons, seams, pockets, collars, and accessories are accurately recreated
- REPRODUCE the exact garment type (shirt, pants, jacket, etc.) as shown in the reference
- MAINTAIN the exact same colors, patterns, and styling as shown in the reference image
- APPLY these details consistently across the entire outfit
- The reference image shows the TRUE representation of what the clothing should look like

CRITICAL REQUIREMENTS (MUST FOLLOW EXACTLY):
- ⚠️ DON'T ADD ANY CLOTHING ITEMS other than the ones in the reference image
- ⚠️ Follow the exact clothing from the reference image regarding color, texture, style, fit, buttons, pockets, etc.
...
- Use the specific background and setting described in the pose instructions below
- The pose description contains the exact background setting to use
- Follow the pose description for all background details, lighting, and atmosphere
- Maintain high-end fashion photography quality
- The background should be slightly blurred to emphasize the subject

POSE REQUIREMENTS:
- Standing upright, completely facing the camera head-on
- Both feet planted firmly on the ground, shoulder-width apart
- Body perfectly centered and straight
- Both arms hanging naturally at the sides
- Hands relaxed and natural, not posed or artificial
- Confident, professional stance
...
- Use the lighting and atmosphere described in the pose instructions

CRITICAL INSTRUCTIONS: COPY THE EXACT CLOTHING FROM THE REFERENCE IMAGE.
- ⚠️ CAREFULLY EXAMINE the reference image to see what the model is actually wearing
- PRECISELY replicate all clothing textures, patterns, colors, and styling shown in the reference image
- MATCH the exact clothing appearance including fabric variations, fit, and styling characteristics
- ENSURE all garment details like seams, stitching, buttons, and styling elements are accurately recreated
- REPRODUCE the exact clothing items and styling as shown in the reference
- MAINTAIN the exact same clothing appearance as shown in the reference image
- APPLY these clothing details consistently across the entire outfit
- The reference image shows the TRUE representation of how the clothing should appear
"""
...
"""
    ...
    def _generate_image_with_prompt(self, prompt, size="1024x1024", quality="standard", style="natural"):
...
- ⚠️ CRITICAL: Pay EXTREMELY close attention to the EXACT COLOR of the quarter-zip pullover sweater from the reference image - this must be perfectly preserved as it's the main product being sold"""
            ...
            logger.error(f"Error applying outfit: {str(e)}")
            raise
