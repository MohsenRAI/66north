...
export function TryOnModal({ isOpen, onClose, selectedColor }: TryOnModalProps) {
  ...
  const handleSwapHead = async () => {
    ...
    try {
      ...
      // Step 2: Headswap
      const referenceImagePath = `bodytypes/headswapper/${analysisResult.metadata.body_type}/arcticfox_headswapper_${analysisResult.metadata.body_type}_${analysisResult.metadata.skin_color}_directfull.png`;
